...
namespace FolderCompare
{
    ...
    {
        IList<FileItem>  lista = new List<FileItem>(10);
        IList<FileItem>  listb = new List<FileItem>(10);
        IList<FileItem>  listc = new List<FileItem>(10); 
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0;i< lista.Count;i++) {
                ...
                grid1.Cell(i + 1, 2).Text  = lista[i].size + " byte"; 
...

        {
            ...
            {
                if (listb.Select(o => o.name).ToList().Contains(lista[i].name))

                {
                    if (File.Exists(lista[i].fullpath))
                    {
                        MessageBox.Show("存在"+ lista[i].fullpath);
                        //File.Delete(comboBox1.Text + "//" + grid1.Cell(i, 1).Text + "");
                    }
                }
            }
